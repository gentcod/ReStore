...
public static class ProductExtensions
{
   ...
   public static IQueryable<Product> Filter(this IQueryable<Product> query, string brands, string types)
   {
        ...
        if (!string.IsNullOrEmpty(brands)) brandList.AddRange(brands.Replace(" ", "").ToLower().Split(",").ToList());

        if (!string.IsNullOrEmpty(types)) typeList.AddRange(types.Replace(" ", "").ToLower().Split(",").ToList());
